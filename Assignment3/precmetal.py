...
def purchase(metal, amt):
    ...
    if account - preciousMetalToDollars(metal, amt) > 0:
        ...
        price = preciousMetalToDollars(metal, amt)
        print('You have purchased {0} oz. of {1} for {2}'.format(amt, metal, price))
        print('You have ${0:.2f} remaining in your account.'.format(account))
        if metal == Au:
            Au_amt += amt
        elif metal == Ag:
            Ag_amt += amt
        ...
        return "You can't purchase more {0}. Insufficient funds. You have {1:.2f} dollars remaining.".format(metal, account)
